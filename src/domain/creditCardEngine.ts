...
export function calculateDueDate(card:CardConfig,statementDate:string){const s=parse(statementDate);if(card.dueDateRule==='manual')return adjustToWeekday(card.manualDueDate||statementDate);if(card.dueDateRule==='days-after-statement'||isSecurityBankCard(card)){s.setDate(s.getDate()+(isSecurityBankCard(card)?21:card.daysAfterStatement));return adjustToWeekday(iso(s))}return adjustToWeekday(iso(dateForDay(s.getFullYear(),s.getMonth()+1,card.fixedDueDay)))}
export function minimumDue(card:CardConfig,balance:number){const fixed=card.minimumFixed,pct=balance*card.minimumPercentage/100;if(balance<=0)return 0;if(card.minimumType==='fixed')return Math.min(balance,fixed);if(card.minimumType==='percentage')return Math.min(balance,pct);if(card.minimumType==='manual')return Math.min(balance,card.manualMinimum);return Math.min(balance,Math.max(fixed,pct))}
...
export function statementFromCycle(card:CardConfig,transactions:CardTransaction[],cycleStart:string,statementDate:string,previousUnpaid=0,id=Date.now()):CardStatement{
  const closedStatementDate=adjustToWeekday(statementDate),end=statementCutoffDate(closedStatementDate),eligible=transactions.filter(t=>t.cardId===card.id&&t.status==='posted'&&t.postedDate>=cycleStart&&t.postedDate<=end)
  const sum=(type:CardTransactionType)=>eligible.filter(t=>t.type===type).reduce((n,t)=>n+t.amount,0)
  const purchases=sum('purchase'),installments=sum('installment'),fees=sum('fee'),interest=sum('interest'),payments=sum('payment'),refunds=sum('refund'),credits=sum('credit')
...
export function computeCard(card:CardConfig,transactions:CardTransaction[],statements:CardStatement[],payments:CardPayment[],today='2026-07-20',reservedInstallmentCredit=0):CardComputed{
  const posted=transactions.filter(t=>t.cardId===card.id&&t.status==='posted'),paymentTotal=payments.filter(p=>p.cardId===card.id&&p.status==='Posted').reduce((n,p)=>n+p.amount,0)
  const transactionBalance=posted.filter(t=>t.type!=='payment').reduce((n,t)=>n+signed(t),0),currentBalance=card.openingBalance+transactionBalance-paymentTotal
  const storedLastStatement=[...statements].filter(s=>s.cardId===card.id).sort((a,b)=>b.statementDate.localeCompare(a.statementDate))[0]
  ...
  const cycleTransactions=posted.filter(t=>t.postedDate>=cycleStart&&t.postedDate<=cycleEnd&&t.type!=='payment'),sum=(type:CardTransactionType)=>cycleTransactions.filter(t=>t.type===type).reduce((n,t)=>n+t.amount,0)
