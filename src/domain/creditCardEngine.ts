...
export function allocatedPaymentsForStatement(payments:CardPayment[],statementId:number|string,statementDate='',asOfDate=''){return payments.reduce((total,payment)=>{
  // A payment made before a statement closes cannot settle that statement.
  // This date guard also neutralizes legacy allocations that were repaired to
  // the wrong (newer) statement id.
  if(statementDate&&String(payment.date||'')<statementDate)return total
  if(asOfDate&&String(payment.date||'')>asOfDate)return total
  return total+(payment.allocations??[]).filter(allocation=>allocation.cycle==='statement'&&String(allocation.statementId)===String(statementId)).reduce((sum,allocation)=>sum+Number(allocation.amount||0),0)
},0)}
