...
export function ConnectedAccountSelect({name="account",defaultValue="",required=false,showBanks=true,showCards=true}:{name?:string;defaultValue?:string;required?:boolean;showBanks?:boolean;showCards?:boolean}) {
  ...
  return <select name={name} required={required} defaultValue={defaultValue}>
    ...
    {showBanks&&<optgroup label="Other"><option value="Cash">Cash</option><option value="eWallet">eWallet</option></optgroup>}
  </select>;
}
